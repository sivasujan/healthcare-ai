"""Doctor recommendation API routes."""

from fastapi import APIRouter, Depends
from sqlalchemy.orm import Session

from app.agents.doctor import run_doctor_recommendation
from app.api.deps import enforce_rate_limit
from app.auth import get_current_user
from app.core.logging import get_logger
from app.database import get_db
from app.middleware.prompt_injection import assert_prompt_safe
from app.models import SavedSearch, User
from app.schemas.common import ApiResponse
from app.schemas.doctor import DoctorRecommendRequest, DoctorRecommendationResult
from app.services.user_service import UserService

router = APIRouter(prefix="/doctor", tags=["Doctor"])

logger = get_logger("api.doctor")


@router.post(
    "/recommend",
    response_model=ApiResponse[DoctorRecommendationResult],
    dependencies=[Depends(enforce_rate_limit)],
)
def recommend(
    payload: DoctorRecommendRequest,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user),
) -> ApiResponse[DoctorRecommendationResult]:
    """Recommend a medical specialty based on symptoms."""
    assert_prompt_safe(payload.symptoms)

    profile = UserService.profile_dict(user)
    profile.update(
        {
            "age": payload.age if payload.age is not None else profile.get("age"),
            "gender": payload.gender or profile.get("gender"),
            "medical_history": payload.medical_history or profile.get("medical_history"),
            "current_medications": payload.current_medications
            or profile.get("current_medications"),
        }
    )

    context_parts = [
        payload.symptoms,
        f"age: {payload.age}" if payload.age is not None else "",
        f"gender: {payload.gender}" if payload.gender else "",
        f"medical history: {payload.medical_history}" if payload.medical_history else "",
        f"medications: {payload.current_medications}" if payload.current_medications else "",
    ]
    full_message = "\n".join(part for part in context_parts if part)

    _, data, model = run_doctor_recommendation(db, user.id, full_message, profile)

    db.add(
        SavedSearch(
            user_id=user.id,
            search_type="doctor",
            query=payload.symptoms[:500],
            result_summary=str(data.get("specialty", ""))[:500],
        )
    )
    db.commit()

    return ApiResponse(success=True, data=DoctorRecommendationResult(**data))
